...
default action, grammar reusability, automatic syntax tree building,
semi-automatic abstract syntax tree building, translation schemes, tree regular
...
of the language associated with each token.
  ...
  lusasoft@LusaSoft:~/src/perl/Eyapp$ eyapp -c lib/Parse/Eyapp/Parse.yp
...
a rule name> (This differs from C<yacc>).
...
and C<examples/eyapplanguageref/Tail.pm> (the module containing the lexical analizer
...
An alternative method is to use inheritance. 
The client inherits the grammar and expands it 
with methods that implement the actions.
Here is the grammar:
...
for the infix to postfix translation:
...
C<YYDelegateaction>. See files C<examples/recycle/NoacYYDelegateaction.eyp>
...
syntactic tree>. Thus, the first print in the section L<Parse::Eyapp> SYNOPSIS example:
  ...
  $ cat -n synopsis.pl
    ...
    17    %%
    18    line:
    19        exp <%name EXPRESION_LIST + ';'>
    20          { $_[1] } /* list of expressions separated by ';' */
    21    ;
    22
    23    /* The %name directive defines the name of the class to which the node being built belongs */
    24    exp:
    25        %name NUM
    26        NUM
    27      | %name VAR
...
production right hand side is parsed. This Perl code receives a C<Parse::Eyapp> object
that describes the grammar. Observe that the code after the C<%namingscheme>
directive is evaluated during the early phases of the compilation of the input 
grammar. As an example, see lines 22-38 below (the example can be found in the directory
...
=item * C<give_lhs_name>: The name of the production is the name of the Left Hand Side of the Production Rule
...
As it is illustrated in the example, the method C<tokennames> of C<Parse::Eyapp>
objects provide a way to give identifier names to tokens that are defined by strings.
When we execute the former program with input C<a=2*-3> we got the following
output:
...
the C<Parse::Eyapp> object, the production index (inside the grammar), the left hand side
symbol and a reference to a list with the symbols in the right hand side. The following code
of  some version of C<give_token_name> exemplifies how a naming scheme handler can be written:
