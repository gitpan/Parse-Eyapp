...
is not really a production rule but an abbreviation. It stands for:
...
The analyzer generated by Eyapp executes C<action()> after all the semantic actions
asssociated with C<alpha> have been executed and before the execution of any of the semantic 
actions associated with C<beta>. Furthermore, I<each symbol on the right hand side
of a production rule has an associated scalar attribute>. The attributes of the symbol
...
See the section 
L<Translation Schemes and the %metatree directive>.

Actions on the 
right hand side counts as symbols. For intermediate actions, the value returned by the C<action> is 
...
the programmer can define which is the default action. A very special
...
The following is a description of the syntax tree for the 
...
scope analysis (scope analysis finds which definition
applies to each object in the source). The program in the synopsis 
...
Such class provides a set of methods to manipulate nodes.
among these methods are C<m> and C<s>. The C<m> and C<s> methods 
resemble the matching and substitution operators for regular
expressions. But instead of regular expressions they work 
with tree transformations, i.e. with C<Parse::Eyapp::YATW> objects.
By calling:
...
applies the whole set of transformations. 
Thus, the former syntax tree for C<"2*-3+b*0;--2\n";> 
becomes:

  EXPRESION_LIST(NUM(TERMINAL[-6]),NUM(TERMINAL[2]))

Thus, the analyzer has been able to deduce - at compile time -
that the computation of these two expressions 
...
=head2 Structure of This Document
...
see the section L<Parse::Eyapp::Node-E<gt>new>
