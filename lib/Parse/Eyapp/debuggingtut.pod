...
since the rule C<e '@' e> has the precedecence of the token C<dummy> which is stronger  
...
which is the same than the token C<&>. The asociativity decides. Since they were declared C<%right>
...
of the automata are called LALR(0) items in the yacc jargon)  is 
C<ds -E<gt> D .';' ds> and 
...
The problem here is that Eyapp/Yapp/Yacc etc. are LALR(1) parsers.
...
a language made of mixed lists IDs and NUMbers :
...
Though not common, it may occur that a reduce-reduce conflict
...
If you supress such comma there is no conflict (try it).
...
is that the automata forgets about it. Look the 
automata transitions in the C<.outputfile>.
By making explict the difference between the first and second C<type>
...
Semantic actions requiring more than one line must go in its own idented block
like in:
