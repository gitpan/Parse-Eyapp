...
I would like to have such feature in a future Perl release. Meanwhile we can do:
...
It works as L<Function insert_method>, only that instead of classes 
...
and proceeds to install the new handler specified through the las argument.
See an example:
...
C<CLASS1::subname>, C<CLASS2::subname>, etc. 'subname').
