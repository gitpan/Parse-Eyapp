...
calculator. Here is the header section:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$   sed -ne '1,11p' Calc.eyp | cat -n
...
as one of the components of the production. Thus the program:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ cat -n intermediateaction2.yp
     1  %%
     2  S:  'a' { $_[1]x4 }.mid 'a' { print "$_[2], $mid, $_[3]\n"; }
     3  ;
     4  %%
     5
     6  use base q{Parse::Eyapp::TailSupport};
     7
     8  __PACKAGE__->lexer(sub {
     9      my($parser)=shift;
    10
    11      for (${$parser->input}) {
    12          s/^[ \t\n]//;
    13          s/^(.)//s and return($1,$1);
    14          return ('', undef);
    15      }
    16    }
    17  );
...
The program C<useintermediateaction2.pl> makes use of the generated parser:

  pl@nereida:~/LEyapp/examples/eyapplanguageref$ cat -n useintermediateaction2.pl                                                      
     1  #!/usr/bin/env perl                                                                                                          
     2  use warnings;                                                                                                                
     3  use strict;
     4  use intermediateaction2;
     5
     6  unshift @ARGV, '--noslurp';
     7  my $parser = intermediateaction2->new;
     8  $parser->main("try input 'aa': ");

The C<main> method is provided by C<Parse::Eyapp::TailSupport>.
When given input C<aa> the execution will produce as output C<aaaa, aaaa, a>. The option
C<-d> used in the following execution activates the debug mode:

  pl@nereida:~/LEyapp/examples/eyapplanguageref$ ./useintermediateaction2.pl -d
  try input 'aa': aa
  ----------------------------------------
  In state 0:
  Stack:[0]
  ...
  Stack:[0,2,4,5]
  Don't need token.
  Reduce using rule 1 (S --> a @1-1 a): aaaa, aaaa, a
...
Following with the calculator example, the body is:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ sed -ne '12,48p' Calc.eyp | cat -n
     1  start:
     2      input { \%s }
     3  ;
     4
...
files C<examples/eyapplanguageref/CalcUsingTail.eyp> (the grammar), 
C<examples/eyapplanguageref/usecalcusingtail.pl> (the client program),
and C<examples/eyapplanguageref/Tail.pm> (the module containing the lexical analyzer
plus error handling and auxiliary subroutines).

=head2 The Lexical Analyzer
...
See below how to write a lexical analyzer (file C<examples/eyapplanguageref/Calc.eyp>):

  pl@nereida:~/LEyapp/examples/eyapplanguageref$ sed -ne '48,$p' Calc.eyp | cat -n
     1  %%
     2  use base q{Parse::Eyapp::TailSupport};
     3
     4  __PACKAGE__->lexer( sub {
     5      my $parser = shift;
     6
     7      for (${$parser->input}) {
     8        m{\G[ \t]*(?:#.*)?}gc;
    ...
    11        m{\G\n}gc                      and do {
    12                                          $parser->tokenline(1);
    13                                          return ("\n", "\n")
    14                                       };
    15        m{\G(.)}gc                     and return ($1,$1);
    16
    17        return('',undef);
    18      }
    19    }
    20  );
    21
    22  # In case is compiled with eyapp option -b
    23  __PACKAGE__->main unless caller;


The subroutine C<lexer> in C<Parse::Eyapp::TailSupport> sets the lexical analyzer 
for the parser to the anonymous sub provided as argument.
a closure. 

=head2 The Error Report Subroutine

The Error Report subroutine is also a parser attribute, 
and must be defined. The module C<Parse::Eyapp::TailSupport>
provides a convenient error handler that is used
in the example in C<examples/Calc.eyp>:
...
The grammar:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ cat List3.yp
  %semantic token 'c'
  %{
  use Data::Dumper;
  %}
...
plus list. 
As it happens when using the C<%tree> directive, I<syntactic tokens> 
are skipped.

When executing the example above but under the C<%tree> directive
the output changes:

  pl@nereida:~/LEyapp/examples/eyapplanguageref$ diff List3.yp List3_tree.yp
  1a2
  > %tree
  14a16
  >
...
a couple of syntax trees:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ ./use_list3_tree.pl
  Try input 'ccdd': ccdd
  $VAR1 = bless( {
    'children' => [
...
Consider the following example:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ cat -n ListWithRefs1.eyp
     1  %semantic token 'c' 'd'
     2  %{
     3  use Data::Dumper;
    ...
    20
    21  use base 'Tail';
    22  __PACKAGE__->lexer(\&Tail::lex);
    23
    24  sub Run {
    25    my ($self) = shift;
    26    return $self->YYParse( yybuildingtree => 1 );
    27  }
...
The solution is to be sure that the attribute is a reference:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ cat -n ListWithRefs.eyp
     1  %semantic token 'c'
     2  %{
     3  use Data::Dumper;
     4  $Data::Dumper::Indent = 1;
    ...
    21
    22  use base 'Tail';
    23  __PACKAGE__->lexer(\&Tail::lex);
    24
    25  sub Run {
    26    my ($self) = shift;
    27    return $self->YYParse( yybuildingtree => 1 );
    28  }
...
and appears in the list of children of C<_PLUS_LIST>:
  ...
  pl@nereida:~/LEyapp/examples/eyapplanguageref$ eyapp ListWithRefs.eyp; ./use_listwithrefs.pl
  Try input 'ccdd': ccdd
  $VAR1 = bless( {
    'children' => [
      bless( { 'children' => [], 'attr' => 'c', 'token' => 'c' }, 'TERMINAL' ),
