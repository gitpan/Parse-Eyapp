...
=head1 Structure of this Document

This document is divided in the following parts:

=over

=item * The introduction attempts to summarize the way C<eyapp>
works. It assumes a reader familiar with parsing techniques
...
If you have trouble with your C<eyapp> program
have a look at section  L<Debugging Parse::Eyapp Programs>.
...
=item * The next part describes (section
L<Miscellaneous Support Functions in Parse::Eyapp::Base>) 
the functions in C<Parse::Eyapp::Base>.
They are support functions for the other modules 
in C<Parse::Eyapp>.
...
C<%metatree> directives. See sections
...
section L<Compiling with eyapp and treereg> for an example.
...
=head1 Debugging C<Parse::Eyapp> Programs
...
In this
section we will discuss three main kind 
...
=head2 Errors During Tree Construction
...
=head2 Understanding the Output of C<yydebug> 
...
=head2 Errors Inside Semantic Actions
...
When using the C<%metatree> directive semantic actions aren't executed. Instead they are 
...
Now we can expand the DAG using C<hexpand>:
...
Notice that the address for C<$z> is the same than the address for C<$x>.
...
=head3 Using C<equal> During Testing
...
=head3 $scope->end_scope with first Arg a Symbol Table and Second 
Argument a Node
...
=head3 $scope->end_scope with first Arg a Symbol Table and Remaining Arguments strings
...
=head1 Miscellaneous Support Functions in C<Parse::Eyapp::Base>
...
Using C<Parse::Eyapp::Base> we can do s.t. similar:
...
Among other
uses, C<push_method> and C<pop_method> make room for some sort of I<dynamic scope privacy> 
for Perl subroutines. 
