...
Each item has three components: the LHS of the production, the number os symbols
...
C<YYState> returns the list of states containing the LALR(1) tables: the 
...
If no subroutine reference is set returns the reference to the current semantic action
...
For a full example, see files examples/recycle/NoacInh.eyp
