...
is based on a deterministic finite automata. 
Each state of the automata I<remembers> what production rules
are candidates to apply.
State 4 contains:
...
C<D> (or something derivable from C<D>) and we 
...
C<D> (or something derivable from C<D>) and since
...
C<ds -E<gt> D .';' ds> and I<shift the semicolon>
